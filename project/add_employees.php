...
<br>
    ...
    btn.onclick = function() {
        ...
        document.querySelector('input[type="submit"]').value = "Add Employees";
    }
...
document.querySelector('input[type="submit"]').onclick = function(event) {
    ...
    formData.append('employeeid', row.cells[0].innerText); // Send beneficiary ID
...
// Get the beneficiary ID from the first cell of the row
// Get the employee ID from the first cell of the row
var employeeId = row.cells[0].innerText; // Corrected variable name
...
fetch('delete_employee.php', {
    ...
    body: 'employeeid=' + encodeURIComponent(employeeId) // Corrected variable name
})
            .then(response => {
                if (response.ok) {
